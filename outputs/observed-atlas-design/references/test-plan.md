...
- **Geometric scaffold:** geometry may carry evidence or page organization. Its area-bearing and line-based families repeat coherently on one route to frame, crop, connect, pace, counterbalance, or expose the subject; not every line must pretend to be a measurement. One inert block plus one faint arc does not retain a scaffold. Detail windows and numbered samples remain optional.
...
- The same subject is copied several times and differs only by opacity, texture, position, sharpness, crop, or surface damage.
- A missing, eroded, or masked region exposes no new structure, material, historical layer, or temporal consequence.
- Old paper and low chroma cover smooth CG, realistic photography, or uniformly synthesized AI detail and are mistaken for European classical craft.
- The subject is staged inside a cinematic concept scene built from an eclipse or moon, ruins, crystals, a realistic sky, a distant horizon, depth of field, or volumetric light.
...
- A request for flatter, more linear, more handmade, or less cluttered treatment converts every meaningful state into the same drawing regime and removes mixed-material contrast.
...
Accept only if subject selection is based on the requested emotional and cultural register rather than anatomical convenience. Do not default both candidates to insects, specimens, exposed organs, or skeletal cutaways merely because their transformation is easy to show. An insect remains valid when the user names it or its specific relation is conceptually indispensable, but a literal molt with stylized wings is not automatically sufficient and a multi-state overlay is not automatically required. The candidates may use different subjects, relations, or transformations, but they must share the same emotional register, temporal and cultural distance, medium and craft character, tonal behavior, and documentary-to-abstract distance unless the prompt explicitly asks for style exploration.

The skill must choose the artistic mechanism from the subject. Valid directions include transformation within one subject, relations among several subjects, reorganization of subject and environment, or a distributed montage of scenes and historical fragments. Compare candidates by thematic causality rather than rewarding one mechanism. Use the distributed-world anchor only when the relation genuinely depends on distribution, never as an automatic way to diversify a second candidate. Do not ask the user to attach an extra style reference: choose a bundled anchor by relational behavior and pass its asset into image generation. Unless the user explicitly requested another visual language, inspect the default classical drawing, engraving, or atlas logic at full size for variable line pressure, credible hatching, construction and correction marks, uneven completion, and coherent joints. Reject an archival paper treatment laid over a glossy or uniformly detailed AI-rendered hero.

This case has now been observed once. The first candidate preserved a quiet, sacred, hand-drawn botanical collage and was directionally accepted. The second changed to the distributed-world anchor and introduced a shared contemporary city, apartment grid, wet street tree root, and East Asian urban context; it became a modern ecological documentary montage and failed the shared visual-language envelope despite being internally coherent. Treat that result as a failing regression example. The new envelope and anchor-selection rules remain statically verified until a fresh task generates, opens, and compares at least two candidates under the revised skill.

### M. Classical-form, planar-space, and fragment-state control

In a fresh task, use:

> 用 OAS 制作一张关于时间痕迹的艺术海报，题材与表现形式不限。

Do not add a style reference or ask for European subject matter. Unless the run explicitly chooses another user-requested style, accept only if the candidates share a credible European classical drawing and paper-production logic through authored line, hatching, underdrawing, correction, incompleteness, uneven detail, and transferred material—not just old paper or muted color. At thumbnail size, every candidate must first read as a flat work assembled on one shallow canvas. Reject a realistic or CG environment organized by horizon, sky, ruins, eclipse, crystals, depth of field, volumetric light, cast shadow, or cinematic perspective even when collage and geometric marks are overlaid.
...
### O. Style-revision role-retention control

Begin with an accepted OAS poster whose meaning depends on at least two subject states, mixed drawing or material regimes, an area-bearing geometric family, and a line-based route. Then use the natural revision request:

> 主体还是不够平面与线条化，可能我更需要偏手绘草稿的欧式风格。

Accept only if the revision changes the subject's edge, line pressure, underdrawing, correction, hatching, and spatial treatment while preserving or re-deriving the prior state information, material counterpoint, geometric mass, long-range curves or axes, and reading path. Geometry may become quieter, but area and line families must still do visible work at thumbnail scale. Quiet space may retain sparse low-contrast route lines when they do not form a competing texture.
