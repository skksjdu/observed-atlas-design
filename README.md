...
- 把“碎片有信息”和“碎片能融合”分开验收：剖面、残片和异态视图还必须通过切面、接合点、结构轴、材料过渡、重叠或线稿延续与主体相连，不能像贴章。
- 不把复杂主体误判为杂乱；保留有意义的主体细节，通过降低几何、纹理、碎片和信号色等辅助层的对比、密度与覆盖范围建立清晰主次。
- 风格修订只改变媒介、线条和显著性，不默认删除原有的状态信息、混合材料、面积几何与长距离曲线；“压低”不等于“清空”。
- 平面化同时检查局部体积一致性：允许断续排线表现形体，但不让单独枝干、构件或剖面通过连续明暗、包裹高光和投影变成雕塑式例外。
- 留白保持安静但不必失联；需要补张力时，只稀疏延续既有轴线、圆弧、节点或间隔家族，不另起一套几何，也不均匀撒点。
- 群体题材先从成员之间的接触、朝向、共同留白、路线或密度推导整体构图，再处理个体；环绕、链条、分支、带状、聚合、编织、汇聚与扩散都可以成立，没有固定群体版式。
- 多人物作品通过绘制阶段、轮廓确定度、线压、排线、转印、裁切和完成度的差异获得手工可信度，避免脸、身体、动作、衣褶和边缘全部同样精致而显出统一 AI 生成感。
...
1.0.0 是当前稳定最终版。源码、安装副本和 ZIP 保持 10 文件交付结构，并使用官方 Skill 校验与逐文件 SHA-256 检查维护一致性；既有回归覆盖建筑、生命、时间与群体人物题材。

近期群体人物回归进一步确认：让人物共同生成整体拓扑和共享空间，比堆叠多个完整人物插图更能表达“群体”；但环绕中央留白只是一次有效构图，不应固化为公式。1.0.0 同时加入多人物表现差异门，抑制统一精致的脸、肌肉、衣褶、动作和排线造成的 AI 合成感。
