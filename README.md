...
- 用缩略图或模糊检查确认一个主读法、受控的次级运动和真正连续的安静区域；浅色但被弧线、网格、噪点和碎片反复穿过的区域不算留白。
...
当前版本处于稳定维护阶段。源码、安装副本和 ZIP 保持 9 文件交付结构，并使用官方 Skill 校验与逐文件 SHA-256 检查维护一致性；建筑任务完成过实际锚点输入、双候选生成和最终渲染审查。

近期真实回归确认了欧式古典、平面浅空间和碎片状态规则能够改善方向，但也暴露了独立的构图噪声问题：主体可以复杂，杂乱来自主体、几何、纹理、碎片和色彩同时高强度发声。当前规则已加入层级竞争、感知留白和辅助层删除测试；仍需后续独立生成确认实际行为。
